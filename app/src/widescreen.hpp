...
//
// The setting (`settings.hpp`) names a mode, not an aspect ratio: RT64 has one
// Expand mode, and this decides when it is on. `Off` is always 4:3, `Missions`
// is Expand only while the dispatcher runs scene `0x03` (the mission), and
// `Always` is Expand everywhere.
//
...
// Sets the window's width from its height and the shape the mode asks for: 4:3
// for `Off` and 16:9 for `Missions` and `Always`. Called once right after the
