...
| `0x801936C9` | `g_chaos_frame` (u8) | the player's Chaos Frame, 0..100: it picks the good and the bad ending, and the scene-`0x13` screen after the credits is the only place the game prints it. Instruction evidence: the game-state initialiser `func_8016C900` bzeroes the flags block at `0x80193698` for `0x3A` bytes and stores `50` here (`0x8016C998`) — the documented starting value; the scene-`0x13` update `func_801B5128` reads this byte and masks it `0x7F` (`0x801B5360`); the scene-script VM opcode `0xFF` stores a script operand here (`0x80171480`, jump table `0x80190BC8`). The community figure for a Project64 symbol is `0x801936A9`; that byte has no read or write in this ROM, and reads 0 at the title while this one reads 50. The app's DEBUG tab reads it live (`app/src/game.hpp` `CHAOS_FRAME_ADDRESS`) | high |
