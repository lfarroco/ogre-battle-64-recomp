...
| 8 | Playable browser build | 🚧 (session 15, 2026-08-31): the boot-stall is **root-caused and fixed**. The session-14 "VI-retrace message-queue deadlock" was a misread — the real cause was a **regression**: the session-8 `yield_self` poll-loop emission was missing from the vendored N64Recomp fork, so thread 3's spin at `0x80075FB8` starved the cooperative scheduler (plus: the scheduler threw "No threads left to run" when all threads blocked, `os*Mesg` drained the whole external backlog per call, and the debug traces slowed headless Chrome ~100×). All fixed (yield_self reimplemented → 99 emissions; scheduler idles; capped drain; traces gated). The web build now boots to a **stable, healthy idle state** (no crash/deadlock, all queues draining at 60 Hz). **Remaining wall:** the game itself never advances past the boot blanking DL — thread 3 (system/audio scheduler) blocks on its own count-1 queue `0x800C6C98` (sender unknown; audio/AI path?), so the frame-producer thread (`func_800893C0`→`func_8008949C`→`osViSwapBuffer`) never runs. See `docs/HANDOFF-2026-08-31-session15.md` |
