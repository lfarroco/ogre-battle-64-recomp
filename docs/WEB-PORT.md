...
| 8 | Playable browser build | 🚧 (session 16, 2026-09-05): session 15's wall description is **superseded** — the frame thread *does* run (frame 1 completes end-to-end: t3 kick → t17 `func_800893C0` → t16 RSP submit → SP/DP-done → kick_next → t5 dispatch), and t3 then **retires** (parks idle, likely `osStopThread`). The precise frame-2 wall: t5's completion dispatch reads key 8 → callback `*(0x800A9E88)` is **NULL** (never set by any executed code; only cb84=swap `0x8008B110` is set), so t5 loops silently and frame 2 is never enqueued. Also fixed this session: **racy worker deaths** — both early browser OOBs and the native SIGBUS traced (via wasm stacks + ASan + lldb watchpoints) to `check_running_queue` dereferencing a **corrupt running-queue entry** (intrusive list lives in game memory; wild boot-time writes poison it). The scheduler is now **self-healing** (link validation + amputation with `[sch-err]` logs in `threadqueue.cpp`/`scheduling.cpp`; AI-buffer range guard in `audio.cpp`), and the snapshot gained a `framedisp` line (kick/dptr/dispatch-key/callbacks). Fast runs now survive to the healthy idle wall (4/4 browser + 2/2 native, zero worker deaths). **Next:** find what sets cb88 (title/overlay code? input-gated?) or who enqueues frame 2. See `docs/HANDOFF-2026-09-05-session16.md`. 🚧 (session 17, 2026-09-05): cb88 proven **unwritable by any static code** (validated ROM-wide lui+sw scan: 0 hits; controls hit) — the setter hasn't run yet, so the stall is elsewhere. Bigger news: **input advances the title past the idle wall** into a new deterministic crash `80071EB0→80075BC0→8017BA60→8019D67C` (3/4 input runs, identical stack; no-input control clean). The chain is fully legitimate (new `titledisp` snapshot: index `0x18`→table[24]=`8017BA54`→`*(0x8018FDC0)`=overlay-B .data vector→`8017BA60`; `func_80073164` returns constant 2 on all paths so `8019D67C` stores through `v0=2`). No wild pointers, no TLB mapping, recompile faithful — title state `0x18` must be HW-unreachable, i.e. state diverged earlier. Next: trace who writes `hu(*D_800C4BBC+4)` and why runs bifurcate (t3 retires vs steady-audio). See `docs/HANDOFF-2026-09-05-session17.md` | 🚧 (session 19, 2026-09-10): **first real game frame rendered in the browser.** The blocker was entirely in the WebGL2 prototype renderer: `ogre_gfx_flush()` blocked the browser main thread on the renderer mutex that `send_dl()` held for a whole DL analysis/execution (illegal on a browser main thread - hard page freeze); `queue_triangles()` re-locked that non-recursive mutex and self-deadlocked on the first real draw command; `read_matrix()` mis-decoded the N64 `Mtx` layout (4x4 s16 integer parts + 4x4 u16 fraction parts), zeroing most matrices so every 3D triangle was rejected; and segment registers stored the full base instead of its high byte. With those fixed the title display list (24 triangles, 1 texrect, 1 fillrect, 24 texture loads, 38 matrices) executes, its draws reach GL, the canvas shows real game content, and frames submit continuously. Sessions 16-18's `cb88` NULL "frame-2 wall" is a **red herring** (that is the boot blanking task; steady-state frame tasks use key 4 -> `cb84 = osViSwapBuffer`) and the "heap-walk wedge" was a downstream symptom. **Next:** `G_MOVEMEM MV_VIEWPORT` is still a no-op (3D geometry lands partly off-screen) and the combiner mux tables need regenerating from RT64's `rt64_color_combiner.h`. See `docs/HANDOFF-2026-09-10-session19.md` |
