...
#
# NOTE: must run BEFORE the .s files are assembled (the Makefile adds this as
# a prerequisite of the object rules), otherwise stale .o files are linked.
set -e
cd "$(dirname "$0")/.."
...
# 1CE040.s: export the label and drop the file-local .L prefix everywhere
# (including branch references like `bnez $v0, .L8019EE70`).
p = 'asm/1CE040.s'
s = open(p).read()
s = re.sub(r'^\s*\.L8019EE70:\s*$', '.globl L8019EE70\nL8019EE70:', s, flags=re.M, count=1)
s = re.sub(r'\.L8019EE70\b', 'L8019EE70', s)
open(p, 'w').write(s)
